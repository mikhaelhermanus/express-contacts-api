...
const search = async (user, request) => {
    ...
    if (request.name) {
        filters.push({
            OR: [ // using OR because users can search by name ==> include by firstname either lastname
                {
                    first_name: {
    ...
    const contacts = await prismaClient.contact.findMany({
        where: {
            AND: filters // using AND operators because users can search by 1 or more params
        },
        take: request.size,
        skip: skip
    })
